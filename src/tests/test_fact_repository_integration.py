"""
Integration test for fact repository functionality, focusing on the review tab.

This test verifies that when a user interacts with the review tab to modify facts or change
their verification status, the system correctly maintains fact IDs and properly moves
facts between repositories.
"""
...
class TestFactRepositoryIntegration:
    """Integration tests for the fact repository functionality."""
    ...
    def test_review_tab_id_preservation(self, setup_test_environment):
        """Test that the review tab correctly preserves fact IDs during operations."""
        gui, fact_repo, rejected_fact_repo, facts_file, rejected_facts_file = setup_test_environment

        # Create a test document with multiple facts
        document_name = f"test_document_{uuid.uuid4().hex[:8]}"

        # Create unique persistent IDs
        fact1_uuid = uuid.uuid4()
        fact1_id = f"fact-{fact1_uuid}"
        
        fact2_uuid = uuid.uuid4()
        fact2_id = f"fact-{fact2_uuid}"
        ...
        fact1 = {
            ...
            "document_name": document_name,
            "source_chunk": 0,
            "original_text": "Test content about semiconductor market",
            "verification_status": "verified",
            "verification_reason": "Initial verification",
            "timestamp": datetime.now().isoformat(),
            "persistent_id": fact1_id
        }
        ...
        # Get all facts for review to verify they exist
        all_facts, _ = gui.get_facts_for_review()
        assert len(all_facts) == 2, "Should have 2 facts for review"

        # Find our test facts
        found_fact1 = None
        found_fact2 = None
        for fact in all_facts:
            if fact.get("persistent_id") == fact1_id:
                found_fact1 = fact
            elif fact.get("persistent_id") == fact2_id:
                found_fact2 = fact
        
        assert found_fact1 is not None, f"Could not find fact1 with persistent_id {fact1_id}"
        assert found_fact2 is not None, f"Could not find fact2 with persistent_id {fact2_id}"

        # Perform a sequence of operations to test ID preservation

        # 1. Modify fact 1 content but keep it verified
        modified_statement1 = "UPDATED: The semiconductor market reached $550B in 2022."
        result1, _ = gui.update_fact_with_transaction(
            fact1_id,
            modified_statement1,
            "verified",
            "Updated with latest market data"
        )
        assert "Fact updated" in result1, f"Expected success in update, got: {result1}"

        # 2. Change fact 2 status to rejected
        result2, _ = gui.update_fact_with_transaction(
            fact2_id,
            fact2["statement"],
            "rejected",
            "Needs further verification"
        )
        assert "Fact updated" in result2, f"Expected success in rejection, got: {result2}"

        # Refresh the data
        gui.refresh_facts_data()
        updated_facts, _ = gui.get_facts_for_review()

        # Find our updated facts
        updated_fact1 = None
        updated_fact2 = None
        ...
        # Verify that fact 1 was updated correctly
        assert updated_fact1 is not None, "Updated fact 1 should exist"
        assert updated_fact1.get("statement") == modified_statement1, "Fact 1 statement should be updated"
        assert updated_fact1.get("verification_status") == "verified", "Fact 1 should remain verified"
        assert updated_fact1.get("persistent_id") == fact1_id, "Persistent ID for fact 1 should be preserved"

        # Verify that fact 2 was moved to rejected correctly
        assert updated_fact2 is not None, "Updated fact 2 should exist"
        assert updated_fact2.get("statement") == fact2["statement"], "Fact 2 statement should be unchanged"
        assert updated_fact2.get("verification_status") == "rejected", "Fact 2 should be rejected"
        assert updated_fact2.get("persistent_id") == fact2_id, "Persistent ID for fact 2 should be preserved"

        # 3. Modify fact 2 and change it back to verified
        reverified_statement = "VERIFIED: AI will transform specific aspects of healthcare diagnostics by 2025."
        result3, _ = gui.update_fact_with_transaction(
            fact2_id,
            reverified_statement,
            "verified",
            "Re-verified with clarified statement"
        )
        assert "Fact updated" in result3, f"Expected success in re-verification, got: {result3}"

        # Refresh and verify the final state
        gui.refresh_facts_data()
        final_facts, _ = gui.get_facts_for_review()

        # Find our final facts
        final_fact1 = None
        final_fact2 = None
        ...
        # Verify that both facts have preserved their IDs
        assert final_fact1 is not None, "Final fact 1 should exist"
        assert final_fact1.get("persistent_id") == fact1_id, "Persistent ID for fact 1 should be preserved"
        
        assert final_fact2 is not None, "Final fact 2 should exist"
        assert final_fact2.get("verification_status") == "verified", "Fact 2 should be verified again"
        assert final_fact2.get("statement") == reverified_statement, "Fact 2 should have updated statement"
        assert final_fact2.get("persistent_id") == fact2_id, "Persistent ID for fact 2 should be preserved"

    def test_fact_id_preserved_during_modification(self, setup_test_environment):
    ...
    def test_fact_id_preserved_when_status_changes(self, setup_test_environment):
    ...
    def test_combined_modification_and_status_change(self, setup_test_environment):
