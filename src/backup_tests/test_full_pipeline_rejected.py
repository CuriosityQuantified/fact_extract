...
import pytest
from src.utils.test_helpers import with_timeout
...
async def test_validator_node_with_rejected_facts():
    ...
    try:
        ...
        print("\nChecking rejected facts in Excel repository...")
        rejected_excel_path = "data/rejected_facts.xlsx"
        if os.path.exists(rejected_excel_path):
            print(f"Reading from {rejected_excel_path}")
            rejected_df = pd.read_excel(rejected_excel_path)
            print(f"Found {len(rejected_df)} rejected facts in Excel file")
        else:
