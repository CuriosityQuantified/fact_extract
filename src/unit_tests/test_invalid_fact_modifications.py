...
def gui_with_sample_facts(setup_test_repositories):
    ...
    # Force save to Excel to ensure data is persisted
    chunk_repo._save_to_excel()
    fact_repo._save_to_excel()
    
    # Create a mock GUI that uses our test repositories
    ...
    # Verify the test fact is in the repository
    test_facts = fact_repo.get_facts(document_name, verified_only=False)
    assert len(test_facts) > 0, "Test fact was not properly stored in repository"
    assert "semiconductor market" in test_facts[0]["statement"], "Test fact statement not found"
    
    # Verify the GUI can access the test fact
    all_facts, _ = gui.get_facts_for_review()
    assert len(all_facts) > 0, "GUI could not retrieve test facts"
    assert "semiconductor market" in all_facts[0]["statement"], "GUI could not retrieve correct test fact"
